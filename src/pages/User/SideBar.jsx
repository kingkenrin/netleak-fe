...
function SideBar({ user, selectMenu, setSelectMenu }) {
  ...
  return (
    <div className="">
      ...
      <div className={`sidebar z-50 fixed bg-[#fafafa] ${isSidebarOpen ? 'left-[0px] overflow-y-hidden' : 'left-[-300px] overflow-auto'}  lg:relative  h-full lg:left-0  p-2  w-[300px]  text-center border-r lg:border-0`}>
        ...
        <div className={`${selectMenu == 'info' && 'bg-[#bbbbbb]'} p-2.5 mt-3 flex items-center rounded-md px-4 duration-300 cursor-pointer hover:bg-[#bbbbbb] text-black`}
          onClick={() => { setSelectMenu('info') }}>
          <UserOutlined />
          <span className="text-[15px] ml-4 font-semibold">Tài Khoản</span>
        </div>

        <div className={`${selectMenu == 'history' && 'bg-[#bbbbbb]'} p-2.5 mt-3 flex items-center rounded-md px-4 duration-300 cursor-pointer hover:bg-[#bbbbbb] text-black`}
          onClick={() => { setSelectMenu('history') }}>
          <HistoryOutlined />
